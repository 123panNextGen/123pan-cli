...
// resolveRedirect 解析重定向链接获取最终下载 URL
func resolveRedirect(rawURL string) (string, error) {
	...
	// 尝试多种模式提取 URL
	patterns := []string{`href='(https?://[^']+)'`, `href="(https?://[^"]+)"`, `url=(https?://[^"&']+)`}
	for _, p := range patterns {
		// 简单字符串查找
		if idx := findPattern(txt, p); idx != -1 {
			// 找到第一个匹配后就返回
		}
	}
...
func findPattern(s, pattern string) int {
	// 简化实现：查找 href='http 这样的模式
	for i := 0; i < len(s); i++ {
		if s[i] == 'h' && i+4 < len(s) && s[i:i+4] == "http" {
			start := i
			// 向前找开始分隔符
			startDelim := byte(0)
			if i > 5 && s[i-5:i] == "href='" {
				startDelim = '\''
				start = i
			} else if i > 5 && s[i-5:i] == "href=\"" {
				startDelim = '"'
				start = i
			}
			// 向后找结束分隔符
			for j := i + 4; j < len(s); j++ {
				if startDelim != 0 && s[j] == startDelim {
					return start // 调用方需要解析
				}
				if s[j] == ' ' || s[j] == '\n' || s[j] == '"' || s[j] == '\'' {
					return start
				}
